...
function SidebarTrigger({
  ...
  return (
    <Button
      ...
      {...props}
    >
      <SidebarIcon weight={open ? "fill" : "duotone"} />
...
const sidebarMenuButtonVariants = cva(
  "peer/menu-button group/menu-button flex w-full items-center gap-2 overflow-hidden truncate rounded-md p-2 text-left text-sm outline-hidden ring-sidebar-ring transition-[width,height,padding] hover:bg-sidebar-accent hover:text-sidebar-accent-foreground focus-visible:ring-2 active:bg-sidebar-accent active:text-sidebar-accent-foreground disabled:pointer-events-none disabled:opacity-50 group-has-data-[sidebar=menu-action]/menu-item:pr-8 aria-disabled:pointer-events-none aria-disabled:opacity-50 data-active:bg-sidebar-accent data-active:font-medium data-active:text-sidebar-accent-foreground data-open:hover:bg-sidebar-accent data-open:hover:text-sidebar-accent-foreground group-data-[collapsible=icon]:size-8! group-data-[collapsible=icon]:p-2! [&>span:last-child]:truncate [&_svg]:size-4 [&_svg]:shrink-0",
  {
    variants: {
      variant: {
...
// ---------------------------------------------------------------------------
// SidebarManager — registry for controlling multiple named sidebars
// ---------------------------------------------------------------------------

interface SidebarManagerContextProps {
  register: (name: string, ctx: SidebarContextProps) => void;
...
function useSidebarManager() {
  const ctx = React.useContext(SidebarManagerContext);
  if (!ctx) {
    throw new Error(
...
function SidebarManagerProvider({ children }: { children: React.ReactNode }) {
  const [sidebars, setSidebars] = React.useState<
    Record<string, SidebarContextProps>
  >({});

  const register = React.useCallback(
    (name: string, ctx: SidebarContextProps) =>
      setSidebars((prev) => ({ ...prev, [name]: ctx })),
    []
  );
  ...
  return (
    <SidebarManagerContext value={value}>{children}</SidebarManagerContext>
  );
}
...
function SidebarManager({
  ...
  const sidebarCtxRef = React.useRef(sidebarCtx);
  const managerRef = React.useRef(manager);

  React.useLayoutEffect(() => {
    sidebarCtxRef.current = sidebarCtx;
    managerRef.current = manager;
  });

  React.useEffect(() => {
    managerRef.current.register(name, sidebarCtxRef.current);
    return () => managerRef.current.unregister(name);
  }, [name]);

  React.useEffect(() => {
    managerRef.current.register(name, sidebarCtx);
  }, [name, sidebarCtx]);
